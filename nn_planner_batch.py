import numpy as np
from planners.planner_batch import Planner
...
class NNPlanner(Planner):
    ...
    def _raw_data(self, start_config):
        ...
        # Dummy Labels
        data['optimal_waypoint_ego_n3'] = np.ones((self.params.batch, 3), dtype=np.float32)
        data['waypoint_horizon_n1'] = np.ones((self.params.batch, 1), dtype=np.float32)
        data['optimal_control_nk2'] = np.ones((self.params.batch, 1, 2), dtype=np.float32)
        return data
